...
import { ApplicationStatus, TaskStatus, UserRole } from '../../common/database/database.types';
import {
  AssignTaskDto,
  AuthLoginDto,
  AuthSignupDto,
...
export class ClientService {
  ...
  getRequests(clientId?: string) {
    ...
    return Array.from(this.db.applications.values())
      ...
      .map((application) => {
        ...
        const status = this.mapRequestStatus(
          task?.status,
          assignment,
  ...
  private isTaskTransitionAllowed(current: TaskStatus, next: TaskStatus) {
    ...
    if (current === TaskStatus.OPEN && next === TaskStatus.IN_PROGRESS)
      return true;
    if (current === TaskStatus.IN_PROGRESS && next === TaskStatus.COMPLETED)
      return true;
    return false;
  }
  ...
  private mapRequestStatus(
    ...
    if (
      assignment?.gig_profile_id === gigProfileId &&
      taskStatus === TaskStatus.IN_PROGRESS
    ) {
      return 'accepted';
    }
    if (assignment && assignment.gig_profile_id !== gigProfileId)
      return 'declined';
    return 'pending';
  }
}

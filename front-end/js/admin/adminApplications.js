...
function render() {
  ...
  if (items.length === 0) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--color-text-muted);padding:var(--spacing-xl);">No applications found.</td></tr>'; return; }
...
function openCreateModal() {
  openModal('Create Application', [formField('Gig Profile ID','gig_profile_id','text',{placeholder:'gpr-1'}), formField('Task ID','task_id','text',{placeholder:'tsk-1'})].join(''), async d => { await createApplication(d); showToast('Created.'); await loadData(); });
}
